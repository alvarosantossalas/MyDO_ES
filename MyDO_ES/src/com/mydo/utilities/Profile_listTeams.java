...
public class Profile_listTeams {
	...
	public String fillListTeamsForAdmin(String id_user_ext) throws Exception {
		ArrayList<String> names = TeamCtrl.getInstance().listAllTeamsForOneUser(id_user_ext);
		String team_name;
		String admin;
		ArrayList<String> members;
		StringBuffer sb = new StringBuffer();
		sb.append("<input type='hidden' name='id_user' value='"+id_user_ext+"'>");
		...
		for (int i = 0; i < names.size(); i++) {
			...
			for (int x = 0; x < members.size(); x++) {
				if (admin.equals(UserCtrl.getInstance().selectNameSurname(members.get(x)))) {
					sb.append("<a href='profile-ext.jsp?id_user_ext="+members.get(x)+"' class='btn btn-primary' style='margin-left: 5px;'>" + UserCtrl.getInstance().selectNameSurname(members.get(x)) + "</a>");
				} else {
					sb.append("<a href='profile-ext.jsp?id_user_ext="+members.get(x)+"' class='btn btn-outline-primary' style='margin-left: 5px;'>" + UserCtrl.getInstance().selectNameSurname(members.get(x)) + "</a>");
				}
			}
			sb.append("</td></tr>");
		}
	...
	public String fillListTeams(String id_user_logado) throws Exception {
